...
Promise.all(tilePromises).then(() => {
    ...
    sync24RadarLiveData();
    setInterval(sync24RadarLiveData, 5000);
}).catch(error => {
...
function radarAircraftScreenPosition(aircraft) {
    const p = aircraft.position || {x:0,y:0};
    // Match 24SPY's own world-to-map conversion so live 24RADAR traffic
    // lands in the same coordinate system as the official 24SPY tiles.
    return transformCoordinates([
        Math.sqrt(Math.abs(Number(p.x) || 0)) / 40,
        Math.sqrt(Math.abs(Number(p.y) || 0)) / 40
    ]);
}

function drawAircraft() {
    const selected = window.radarSelectedCallsign || null;
    Object.values(aircraftData || {}).forEach(aircraft => {
        const [x,y] = radarAircraftScreenPosition(aircraft);
        const heading = Number(aircraft.heading || 0);
        const emergency = !!aircraft.isEmergencyOccuring;
        const selectedNow = aircraft.callsign === selected;

        ctx.save();
        ctx.translate(x,y);
        ctx.rotate((heading * Math.PI) / 180);

        // PTFS/24SPY-style aircraft symbol, drawn natively so no extra image
        // dependency is needed.
        ctx.beginPath();
        ctx.moveTo(0,-12); ctx.lineTo(2,-4); ctx.lineTo(9,3);
        ctx.lineTo(8,5); ctx.lineTo(2,3); ctx.lineTo(2,8);
        ctx.lineTo(4,11); ctx.lineTo(4,12); ctx.lineTo(0,10);
        ctx.lineTo(-4,12); ctx.lineTo(-4,11); ctx.lineTo(-2,8);
        ctx.lineTo(-2,3); ctx.lineTo(-8,5); ctx.lineTo(-9,3);
        ctx.lineTo(-2,-4); ctx.closePath();
        ctx.fillStyle = emergency ? "#ff4545" : (aircraft.isOnGround ? "#b7b7b7" : "#3b6cec");
        ctx.fill();

        if(selectedNow){
            ctx.beginPath(); ctx.arc(0,0,18,0,Math.PI*2);
            ctx.strokeStyle="#3b6cec"; ctx.lineWidth=2; ctx.stroke();
        }
        ctx.restore();

        // Label block follows the same compact map-label idea as 24SPY.
        ctx.save();
        ctx.font = "bold 11px Arial";
        ctx.fillStyle = emergency ? "#ff5555" : "#ffffff";
        ctx.strokeStyle = "rgba(19,19,22,.9)";
        ctx.lineWidth = 3;
        const label = aircraft.callsign || "UNKNOWN";
        const info = `FL${String(Math.max(0,Math.round((Number(aircraft.altitude)||0)/100))).padStart(3,"0")} · ${Math.round(Number(aircraft.speed)||0)}kt`;
        ctx.strokeText(label, x+15, y-3);
        ctx.fillText(label, x+15, y-3);
        ctx.font = "9px Arial";
        ctx.fillStyle = "#b8b8b8";
        ctx.strokeText(info, x+15, y+9);
        ctx.fillText(info, x+15, y+9);
        ctx.restore();
    });
}
...
async function sync24RadarLiveData() {
    try {
        ...
        const next = {};
        Object.entries(live.aircraft || {}).forEach(([callsign, d]) => {
            next[callsign] = {
                callsign,
                ...
                speed: Number(d.groundSpeed ?? d.speed ?? 0),
                groundSpeed: Number(d.groundSpeed ?? d.speed ?? 0),
                wind: d.wind || "",
                playerName: d.playerName || "",
                isOnGround: !!d.isOnGround,
                isEmergencyOccuring: !!d.isEmergencyOccuring,
                destination: d.destination || d.arrivalAirport || d.arrival || "",
                origin: d.origin || d.departureAirport || d.departure || ""
            };
        });
        aircraftData = next;

        // The 24SPY controller UI continues to consume its normal controller
        // schema, but the source is now the same 24RADAR endpoint.
        window.__radarLiveControllers = Array.isArray(live.controllers) ? live.controllers : [];
        draw();

        const timeEl=document.querySelector("#mapUpdateTime .time");
        if(timeEl) timeEl.textContent=" "+getTime();
        const countEl=document.querySelector(".atc-online .online-number");
        if(countEl) countEl.textContent=window.__radarLiveControllers.length;
    } catch(err) {
        console.error("24RADAR live sync failed:",err);
    }
}
...
async function fetchATCDataAndUpdate() {
    ...
    if (devMode) {
        ...
        data = window.__radarLiveControllers || [];
        if (!data.length) {
            try {
                const liveResponse = await fetch('/api/live', {cache:'no-store'});
                if (liveResponse.ok) {
                    const livePayload = await liveResponse.json();
                    data = Array.isArray(livePayload.controllers) ? livePayload.controllers : [];
                }
            } catch (e) {
                console.warn('24RADAR controller sync failed', e);
            }
        }
    }
...
// 24RADAR aircraft selection, layered onto the original 24SPY canvas.
window.radarSelectedCallsign = null;
function radarDistanceToAircraft(mx,my,aircraft){
    const p=radarAircraftScreenPosition(aircraft);
    return Math.hypot(p[0]-mx,p[1]-my);
}
function radarGetSelectedAircraft(){
    return window.radarSelectedCallsign ? (aircraftData||{})[window.radarSelectedCallsign] : null;
}
function radarOpenAircraftPanel(ac){
    if(!ac) return;
    window.radarSelectedCallsign=ac.callsign;
    const panel=document.getElementById("radarAircraftPanel");
    if(panel) panel.classList.add("show");
    const set=(id,v)=>{const e=document.getElementById(id); if(e)e.textContent=v;};
    set("radarPanelCallsign",ac.callsign||"Aircraft");
    set("radarPanelPilot",ac.playerName||"—");
    set("radarPanelType",ac.aircraftType||"—");
    set("radarPanelAlt",Math.round(ac.altitude||0)+" ft");
    set("radarPanelHdg",Math.round(((ac.heading||0)%360+360)%360)+"°");
    set("radarPanelSpeed",Math.round(ac.speed||0)+" kt");
    set("radarPanelState",ac.isEmergencyOccuring?"EMERGENCY":(ac.isOnGround?"GROUND":"AIRBORNE"));
    set("radarPanelOrigin",(ac.origin||"—").toString().split(/[ —-]/)[0].toUpperCase());
    set("radarPanelDest",(ac.destination||"DEST —").toString().split(/[ —-]/)[0].toUpperCase());
    // Only show a percentage when both route endpoints are actually available.
    const pct = Number.isFinite(ac.progress) ? Math.max(0,Math.min(100,ac.progress)) : null;
    const fill=document.getElementById("radarProgressFill");
    if(fill) fill.style.width=(pct==null?0:pct)+"%";
    set("radarPanelPct",pct==null?"—":Math.round(pct)+"%");
    set("radarPanelDistance",ac.destination ? "Destination supplied by live data" : "Destination unavailable");
    draw();
}
function radarCloseAircraftPanel(){
    window.radarSelectedCallsign=null;
    const panel=document.getElementById("radarAircraftPanel");
...
canvas.addEventListener("click",function(ev){
    ...
    Object.values(aircraftData||{}).forEach(ac=>{
        const d=radarDistanceToAircraft(mx,my,ac);
        if(d<bestD){bestD=d;best=ac;}
    });
    if(best) radarOpenAircraftPanel(best);
});
document.getElementById("radarPanelClose")?.addEventListener("click",radarCloseAircraftPanel);
